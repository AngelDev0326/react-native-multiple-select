      ...
      return (
        <View
          style={[
            styles.selectedItem,
            {
              width: item[displayKey].length * styleTextTag.fontSize / 2.1 + styleTextTag.fontSize * 3.8,
        ...
        >
          ...
          <TouchableOpacity onPress={() => { this._removeItem(item); }}>
            <Icon name="close-circle" style={{ color: tagRemoveIconColor, fontSize: 22, /* marginLeft: 10*/ }}
            />
          </TouchableOpacity>
        </View>
      );
      ...
      >
        {selector ? (
          ...
          <View>
            ...
            {!single && !hideTags && selectedItems.length && <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>{this._displaySelectedItems()}</View>}
          </View>
        )}
      </View>
    );
