...
export default class MultipleSelect extends Component {
  static propTypes = {
    ...
    selectAllButtonColor: PropTypes.string,
    selectAllButtonText: PropTypes.string,
    deselectAllButtonText: PropTypes.string,
    ...
    selectedTitleAsCount: PropTypes.bool,
  ...
  _getSelectLabel = () => {
    const { selectText, single, selectedItems, displayKey, selectedTitleAsCount, selectedText } = this.props;
      ...
      return (
        <View
          style={[
            styles.selectedItem,
            {
              width: item[displayKey].length * styleTextTag.fontSize / 2.1 + styleTextTag.fontSize * 4,
              maxWidth: '100%',
        ...
        >
          <Text
            style={[
              { flex: 1, color: tagTextColor, fontSize: 15 },
  ...
  _selectAllSelection = () => {
    const { items, uniqueKey, onSelectedItemsChange } = this.props;
    let selectedItems = items.map(one => one[uniqueKey])
    onSelectedItemsChange([...selectedItems]);
    this._submitSelection();
  };

  _deselectAllSelection = () => {
      ...
      >
        <View>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Text
              style={[
                { flex: 1, fontSize: 16, paddingTop: 5, paddingBottom: 5 },
            ...
            {this._itemSelected(item) && <Icon name="check" style={{ fontSize: 20, color: selectedItemIconColor }} />}
          </View>
        </View>
      </TouchableOpacity>
    );
  ...
  _getRowNew = item => (
    <TouchableOpacity disabled={item.disabled} onPress={() => this._addItem(item)} style={{ paddingLeft: 20, paddingRight: 20 }}>
      <View>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text
            style={[
  ...
  _renderItems = () => {
    const { canAddItems, items, fontFamily, uniqueKey, selectedItems, flatListProps, styleListContainer, removeSelected, noItemsText } = this.props;
    ...
    if (renderItems.length) {
      ...
      itemList = (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text
            style={[
              { flex: 1, marginTop: 20, textAlign: 'center', color: colorPack.danger },
              fontFamily ? { fontFamily } : {}
            ]}
          >
            {noItemsText}
  ...
  render() {
    ...
    return (
      <View style={[{ flexDirection: 'column' } && styleMainWrapper && styleMainWrapper]}>
        {selector ? (
          <View
            style={[
            ...
            <View style={{ flexDirection: 'column', backgroundColor: '#fafafa' }}>
              ...
              {!single && !hideSubmitButton && (
                <View style={{flexDirection: "row", width: "100%"}}>
                  <TouchableOpacity onPress={() => items.length == selectedItems.length ? this._deselectAllSelection() : this._selectAllSelection()} style={[styles.button, { backgroundColor: selectAllButtonColor }]}>
                    <Text style={[styles.buttonText, fontFamily ? { fontFamily } : {}]}>
                      {items.length == selectedItems.length ? deselectAllButtonText : selectAllButtonText}
                    </Text>
                  </TouchableOpacity>
                  <View style={{backgroundColor: "white", width: 1}} />
                  <TouchableOpacity onPress={() => this._submitSelection()} style={[styles.button, { backgroundColor: submitButtonColor }]}>
                    <Text style={[styles.buttonText, fontFamily ? { fontFamily } : {}]}>
                      {submitButtonText}
                    </Text>
                  </TouchableOpacity>
              ...
              >
                <TouchableWithoutFeedback onPress={this._toggleSelector}>
                  <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Icon name={hideSubmitButton ? 'menu-right' : 'menu-down'} style={[styles.indicator, styleIndicator && styleIndicator]} />
                  </View>
